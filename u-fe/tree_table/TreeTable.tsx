...
export type TreeColumnName = "__tree_column__";
...
export type ColumnDefinitions = {
  ...
  columns: { [regularColumnName: string]: NonTreeColumnDefinition };
};
...
export type Sort = {
  order: SortOrder;
  columnName: TreeColumnName | string;
};
...
export function TreeTable(props: {
  ...
  sortColumnName: TreeColumnName | string | null;
  ...
  const ctx = useMemo(() => {
    ...
    ctx.updateSortState(props.sortColumnName, props.sortOrder);
  ...
  useEffect(() => {
    ctx.updateSortState(props.sortColumnName, props.sortOrder);
    ctx.resortRows();
  }, [props.sortColumnName, props.sortOrder, ctx]);
  ...
  const columnsElements = columns.map((column, columnIDX) => {
    ...
    const sortOrder =
      columnName === props.sortColumnName ? props.sortOrder : null;
      ...
      if (sortOrder === "asc") {
        ...
        return (
          <Icon
            ...
            onClick={() => props.onSortChange({ columnName, order: "desc" })}
          />
        );
        ...
        return (
          <Icon
            size={16}
            className="mx-2 cursor-pointer"
            onClick={() => props.onSortChange({ columnName, order: "asc" })}
          />
        );
      } else {
        return (
...
export type SortState = {
  sortFn: SortFn;
  sortColumnName: TreeColumnName | string;
};
...
class TreeTableCtx {
  ...
  updateSortState(
    columnName: TreeColumnName | string | null,
  ...
  expandRow(rowIDX: number) {
    ...
    for (const column of this.columns) {
      if (column.t === "numeric_value_column") {
        if (this.sortState?.sortColumnName === column.name) {
...
function useMakeInternalColumns(
  ...
  return useMemo(() => {
    const result: Array<ColumnInternal> = [
      {
        t: "tree",
        name: "__tree_column__",
        c: columnDefinitions.treeColumn,
      },
    ];

    for (const [name, column] of Object.entries(columnDefinitions.columns)) {
      switch (column.t) {
        case "numeric_value_column":
          {
            result.push({
