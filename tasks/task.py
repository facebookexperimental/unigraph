...
@check_app.command("copyright")
def check_copyright() -> None:
    """Check that source files contain a Meta/Facebook copyright header."""
    failures: list[str] = []
    for path in sorted(ROOT.rglob("*")):
        ...
        if path.suffix.lower() in EXCLUDED_EXTENSIONS:
            continue
        try:
            header = path.read_text(encoding="utf-8", errors="strict")[:2048]
        except (UnicodeDecodeError, ValueError):
            continue
        if COPYRIGHT_HEADER not in header:
            failures.append(str(rel))
    if failures:
        print(f"\n\033[1;31m{len(failures)} file(s) missing copyright header:\033[0m\n")
        for f in failures:
            print(f"  {f}")
        print()
        raise typer.Exit(1)
    else:
        print("\033[32mAll source files have copyright headers.\033[0m")
