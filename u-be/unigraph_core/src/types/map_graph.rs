...
pub struct GraphNode {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub edges_dynamic: Option<Vec<DynamicEdge>>,
    ...
    pub fn to_array_graph(&self) -> Result<ArrayGraph> {
        ...
        for node_name in node_names_ordered.iter_names() {
            ...
            if let Some(node) = self.nodes.get(node_name) {
                ...
                for dynamic in node.edges_dynamic.iter().flatten() {
                ...
                sizes.push(node.size.unwrap_or_default() as f32);
            } else {
...
impl GraphNode {
    ...
    pub fn arrow_iter<'a>(&'a self) -> impl Iterator<Item = NamedArrow> + 'a {
        ...
        let tagged = self.edges_tagged.iter().flatten().map(|(tag, edges)| {
        ...
        let dynamic = self.edges_dynamic.iter().flatten().map(|d| {
