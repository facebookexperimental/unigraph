...
//! satisfy every condition in the filter — properties and incoming edges are
//! ANDed together, as are the entries within each.
//!
//! The two passes are ordered so the expensive one runs on the smallest set:
//!
//! ```text
...
//!   edges           skipped entirely when no edge conditions are set
//! ```
//!
...
pub fn filter_entry_points(ag: &ArrayGraph, filter: &EntryPointsFilter) -> Vec<NodeIDX> {
    ...
    retain_by_incoming_edges(ag, candidates, filter)
}
...
/// Drop candidates whose incoming edges don't carry every requested tag and
/// dynamic type key.
///
/// Returns the candidates untouched when no edge conditions are set, so a
/// properties-only filter never forces the reverse graph to be built.
fn retain_by_incoming_edges(
    ag: &ArrayGraph,
    mut candidates: Vec<NodeIDX>,
    filter: &EntryPointsFilter,
) -> Vec<NodeIDX> {
    if !filter.has_incoming_edge_conditions() {
        return candidates;
    }

    let reverse = ag.edge_view(GraphStructure::Reverse);
    candidates.retain(|&node_idx| matches_incoming_edges(&reverse, node_idx, filter));
    candidates
}

fn matches_incoming_edges(
    reverse: &EdgeGraphView<'_>,
    node_idx: NodeIDX,
    filter: &EntryPointsFilter,
) -> bool {
    filter
        .incoming_tags
        .iter()
        .all(|tag| has_incoming_tag(reverse, node_idx, tag))
        && filter
            .incoming_dynamic_type_keys
            .iter()
            .all(|type_key| has_incoming_dynamic_type(reverse, node_idx, type_key))
}

fn has_incoming_tag(reverse: &EdgeGraphView<'_>, node_idx: NodeIDX, tag: &str) -> bool {
    incoming_edge_metadata(reverse, node_idx)
        .any(|meta| matches!(meta, EdgeMeta::Tagged { tag: actual } if actual == tag))
}

fn has_incoming_dynamic_type(
    reverse: &EdgeGraphView<'_>,
    node_idx: NodeIDX,
    type_key: &str,
) -> bool {
    incoming_edge_metadata(reverse, node_idx).any(
        |meta| matches!(meta, EdgeMeta::Dynamic { type_key: actual, .. } if actual == type_key),
    )
}

/// Metadata of the node's incoming tagged and dynamic edges.
///
...
fn incoming_edge_metadata<'a>(
    reverse: &'a EdgeGraphView<'a>,
    node_idx: NodeIDX,
...
mod tests {
    ...
    const TEST_GRAPH: &str = r#"{
      "nodes": {
        ...
        "beta":    { "properties": { "budget_type": "ROUTE" } },
        "gamma":   { "properties": { "budget_type": "PAGE", "team": "ads" } },
        "delta":   { "properties": { "budget_type": "ROUTE", "team": "ads" } },
    ...
    fn filter(
        ...
        incoming_tags: &[&str],
        incoming_dynamic_type_keys: &[&str],
    ) -> EntryPointsFilter {
        EntryPointsFilter {
            properties: properties
                ...
                .collect(),
            incoming_tags: incoming_tags.iter().map(|t| t.to_string()).collect(),
            incoming_dynamic_type_keys: incoming_dynamic_type_keys
                .iter()
                .map(|k| k.to_string())
                .collect(),
        }
    ...
    fn all_cases() -> Vec<(&'static str, EntryPointsFilter)> {
        vec![
            ...
            (
                "budget_type=ROUTE",
                filter(&[("budget_type", Some("ROUTE"))], &[], &[]),
            ),
            (
                "budget_type=ROUTE + team=ads",
                filter(
                    &[("budget_type", Some("ROUTE")), ("team", Some("ads"))],
                    &[],
                    &[],
                ),
            ),
            (
                "has budget_type",
                filter(&[("budget_type", None)], &[], &[]),
            ),
            ("team=core", filter(&[("team", Some("core"))], &[], &[])),
            ("incoming tag lazy", filter(&[], &["lazy"], &[])),
            ("incoming tag eager", filter(&[], &["eager"], &[])),
            ("incoming dynamic rc:gk", filter(&[], &[], &["rc:gk"])),
            (
            ...
            (
                ...
                filter(&[("budget_type", Some("ROUTE"))], &["lazy"], &[]),
            ),
            (
                "budget_type=PAGE + incoming tag lazy",
                filter(&[("budget_type", Some("PAGE"))], &["lazy"], &[]),
            ),
            (
            ...
            (
                ...
                filter(&[("budget_type", Some("NOPE"))], &[], &[]),
            ),
            ("unknown tag", filter(&[], &["nope"], &[])),
            ("unknown dynamic type", filter(&[], &[], &["nope"])),
            (
                "known property + unknown tag",
                filter(&[("budget_type", Some("ROUTE"))], &["nope"], &[]),
            ),
        ]
    }
...
"
    ...
    /// An excluded edge must not satisfy an incoming-edge condition, even when
    /// the node it points at is still reachable some other way. `epsilon` keeps
    /// its directed edge from `root`, so this isolates edge exclusion from the
    /// node-level reachability check above.
    #[test]
