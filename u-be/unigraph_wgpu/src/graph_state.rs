...
impl GraphState {
    ...
    pub fn recalculate_adjusted_sizes(&mut self) {
        ...
        sort_vec_f32(&mut all_sizes);

        let min_size = all_sizes[0];
        let max_size = all_sizes[all_sizes.len() - 1];

        let mut adjusted_sizes = Vec::with_capacity(all_sizes.len());
        ...
        for size in all_sizes {
            ...
            adjusted_sizes.push(adjusted_size);
        }
    }
    ...
    fn forces_pull_towards_center(&self) -> Vec<Vec2> {
        ...
        for (idx, node) in self.node_attributes.iter().enumerate() {
            ...
            forces[idx] = forces[idx] + Vec2 { x: fx, y: fy };
        }
        forces
    }
...
fn sort_vec_f32(vec: &mut Vec<f32>) {
