...
pub async fn start(graphite_graph_json_file_path: &Option<String>) -> Result<()> {
    let map_graph = if let Some(file_path) = graphite_graph_json_file_path {
        ...
        unigraph_core::make_test_graph()?
    };

    let map_graph = Arc::new(map_graph);

    // build our application with a single route
    let app = Router::new().route(
        "/",
        get(move || {
            let map_graph = Arc::clone(&map_graph);
            async move { Html(html(&map_graph).unwrap()) }
        }),
    );
...
fn html(map_graph: &MapGraph) -> Result<String> {
